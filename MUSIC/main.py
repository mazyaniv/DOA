...
if __name__ == "__main__":
    ...
    for i in range(len(SNR_space)):
        for j in range(len(N_a)):
            ...
            Error1[i, j], Error2[i, j] = music_algorithm(my_parameters,delta)
    ...
    plt.grid()
    plt.title(f"RMSE for Snap={my_parameters.snapshot}, Monte={my_parameters.monte}, Res={my_parameters.Reso}")
    plt.ylabel("RMSE (Deg.)")
    plt.xlabel("SNR [dB]")
